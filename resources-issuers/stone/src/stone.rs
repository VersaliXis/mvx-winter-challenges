...
{   
    ...
    // Stores each issued token associeted with the address of the endpoint issueTokenStone caller
    ...
    /// Allows to issue a STONE-xx token. Uses a callback to save the token id
    #[endpoint(issueTokenStone)]
    #[payable("EGLD")]
    fn issue_token_stone(&self, amount: BigUint) {
        ...
        let token_display_name = ManagedBuffer::from("StoneToken");
        let token_ticker = ManagedBuffer::from("STONE");
    ...
    #[endpoint(burnTokenStone)]
    fn burn_token_stone(&self, token: TokenIdentifier, amount: BigUint) {
        require!(self.issued_tokens().contains(&token), "Invalid token ID");
    ...
    #[endpoint(mintAndSendTokenStone)]
    fn mint_and_send_token_stone(&self, token: &TokenIdentifier, amount: &BigUint, to: &ManagedAddress){
        self.mint_token_stone(token, amount);
        self.send().direct_esdt(to, token, 0u64, amount);
    }

    /// Allows to mint a STONE-xx token
    /// Must have called setLocalRoles before
    #[endpoint(mintTokenStone)]
    fn mint_token_stone(&self, token: &TokenIdentifier ,amount: &BigUint) {
