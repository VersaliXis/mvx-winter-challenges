...
{   
    ...
    // Stores each issued token associeted with the address of the endpoint issueTokenWood caller
    ...
    /// Allows to issue a WOOD-xx token. Uses a callback to save the token id
    #[endpoint(issueTokenWood)]
    #[payable("EGLD")]
    fn issue_token_wood(&self, amount: BigUint) {
        ...
        let token_display_name = ManagedBuffer::from("WoodToken");
        let token_ticker = ManagedBuffer::from("WOOD");
    ...
    #[endpoint(burnTokenWood)]
    fn burn_token_wood(&self, token: TokenIdentifier, amount: BigUint) {
        require!(self.issued_tokens().contains(&token), "Invalid token ID");
    ...
    #[endpoint(mintAndSendTokenWood)]
    fn mint_and_send_token_wood(&self, token: &TokenIdentifier, amount: &BigUint, to: &ManagedAddress){
        self.mint_token_wood(token, amount);
        self.send().direct_esdt(to, token, 0u64, amount);
    }

    /// Allows to mint a WOOD-xx token
    /// Must have called setLocalRoles before
    #[endpoint(mintTokenWood)]
    fn mint_token_wood(&self, token: &TokenIdentifier ,amount: &BigUint) {
