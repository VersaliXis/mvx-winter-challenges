...
{   
    ...
    // Stores each issued token associeted with the address of the endpoint issueTokenFood caller
    ...
    /// Allows to issue a FOOD-xx token. Uses a callback to save the token id
    #[endpoint(issueTokenFood)]
    #[payable("EGLD")]
    fn issue_token_food(&self, amount: BigUint) {
        ...
        let token_display_name = ManagedBuffer::from("FoodToken");
        let token_ticker = ManagedBuffer::from("FOOD");
    ...
    #[endpoint(burnTokenFood)]
    fn burn_token_food(&self, token: TokenIdentifier, amount: BigUint) {
        require!(self.issued_tokens().contains(&token), "Invalid token ID");
    ...
    #[endpoint(mintAndSendTokenFood)]
    fn mint_and_send_token_food(&self, token: &TokenIdentifier, amount: &BigUint, to: &ManagedAddress){
        self.mint_token_food(token, amount);
        self.send().direct_esdt(to, token, 0u64, amount);
    }

    /// Allows to mint a FOOD-xx token
    /// Must have called setLocalRoles before
    #[endpoint(mintTokenFood)]
    fn mint_token_food(&self, token: &TokenIdentifier ,amount: &BigUint) {
